...
export default function AramaKlient({ initialSorgu = '', initialSonuc = null }) {
  const searchParams = useSearchParams()
  const router = useRouter()
  const sorgu = searchParams.get('q') || ''
  const gorselArama = searchParams.get('gorsel') === '1'

  const [inputDeger, setInputDeger] = useState(initialSorgu)
  const [yukleniyor, setYukleniyor] = useState(false)
  // Sunucudan gelen sonucu başlangıç değeri olarak kullan — hydrasyon flaşı yok
  const [sonuc, setSonuc] = useState(initialSonuc)
  const [hata, setHata] = useState(null)
  const oncekiSorgu = useRef(initialSorgu)
  ...
  useEffect(() => {
    ...
    // URL'deki sorgu değiştiğinde yeni arama yap (sunucu sonucuyla başladıysak ilk fırlamayı atla)
    if (sorgu && sorgu !== oncekiSorgu.current) {
      oncekiSorgu.current = sorgu
      ara(sorgu)
    }

    if (!sorgu && !gorselArama) {
      setSonuc(null)
      setHata(null)
    }
  }, [sorgu, gorselArama])
